...
### Never analyse a same-day Explore export (rule since 2026-09-25)

GA4 Explore does not finish processing a day until well after it ends. On 2026-09-25, an export of the H15 control traffic taken the same day showed 3 ChatGPT-User and 2 Claude-User hits. The same query, exported again a few hours later that day, showed 5 and 5, exactly one per round. How long the lag lasts is unknown, which is why the rule waits for a full closed day. Read at face value, the first export would have reported five instrument misses that never happened. Every export used for a verdict covers days that ended **at least 24 hours before** the export, and the export date is recorded next to the numbers. Realtime is exempt, because it is a live 30-minute view and not a daily table.
